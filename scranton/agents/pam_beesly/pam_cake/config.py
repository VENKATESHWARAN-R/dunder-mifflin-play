"""
Configs module for Pam Cake agent.
"""
...
from pam_beesly.pam_cake.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    def agent_instruction(context: ReadonlyContext) -> str:
        return get_agent_instruction(
            os.getenv("PAM_CAKE_INSTRUCTION_VERSION", "v1")
        )
    @property
    def agent_description(self) -> str:
        return get_agent_description(
            os.getenv("PAM_CAKE_DESCRIPTION_VERSION", "v1")
        )
    model_id: str = field(
        default_factory=lambda: os.getenv("PAM_CAKE_MODEL_ID", "gemini-2.0-flash-lite")
    )

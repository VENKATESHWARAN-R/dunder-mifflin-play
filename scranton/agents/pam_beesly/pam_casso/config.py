"""
Configs module for Pam Casso agent.
"""
...
from pam_beesly.pam_casso.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    agent_instruction: str = field(
        default_factory=lambda: get_agent_instruction(
            os.getenv("PAM_CASSO_INSTRUCTION_VERSION", "v1")
        )
    )
    agent_description: str = field(
        default_factory=lambda: get_agent_description(
            os.getenv("PAM_CASSO_DESCRIPTION_VERSION", "v1")
        )
    )
    model_id: str = field(
        default_factory=lambda: os.getenv("PAM_CASSO_MODEL_ID", "gemini-2.0-flash-lite")
    )

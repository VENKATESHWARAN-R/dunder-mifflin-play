"""
Configs module for William Charles Schneider agent.
"""
...
from creed_bratton.william_charles_schneider.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    agent_instruction: str = field(
        default_factory=lambda: get_agent_instruction(
            os.getenv("WILLIAM_INSTRUCTION_VERSION", "v1")
        )
    )
    agent_description: str = field(
        default_factory=lambda: get_agent_description(
            os.getenv("WILLIAM_DESCRIPTION_VERSION", "v1")
        )
    )
    model_id: str = field(
        default_factory=lambda: os.getenv("WILLIAM_MODEL_ID", "gemini-2.0-flash-lite")
    )

"""
Configs module for Golden Face agent.
"""
...
from jim_halpert.golden_face.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    def agent_instruction(context: ReadonlyContext) -> str:
        return get_agent_instruction(
            os.getenv("GOLDEN_FACE_INSTRUCTION_VERSION", "v1")
        )
    @property
    def agent_description(self) -> str:
        return get_agent_description(
            os.getenv("GOLDEN_FACE_DESCRIPTION_VERSION", "v1")
        )
    model_id: str = field(
        default_factory=lambda: os.getenv("GOLDEN_FACE_MODEL_ID", "gemini-2.0-flash-lite")
    )

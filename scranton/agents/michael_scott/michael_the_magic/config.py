"""
Configs module for Michael The Magic agent.
"""
...
from michael_scott.michael_the_magic.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    def agent_instruction(context: ReadonlyContext) -> str:
        return get_agent_instruction(
            os.getenv("MICHAEL_MAGIC_INSTRUCTION_VERSION", "v1")
        )
    @property
    def agent_description(self) -> str:
        return get_agent_description(
            os.getenv("MICHAEL_MAGIC_DESCRIPTION_VERSION", "v1")
        )
    model_id: str = field(
        default_factory=lambda: os.getenv("MICHAEL_MAGIC_MODEL_ID", "gemini-2.0-flash-lite")
    )

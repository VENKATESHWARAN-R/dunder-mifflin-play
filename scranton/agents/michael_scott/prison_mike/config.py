"""
Configs module for Prison Mike agent.
"""
...
from michael_scott.prison_mike.prompts import ( # pylint: disable=E0401
...
class AgentConfig:
    ...
    def agent_instruction(context: ReadonlyContext) -> str:
        return get_agent_instruction(
            os.getenv("PRISON_MIKE_INSTRUCTION_VERSION", "v1")
        )
    @property
    def agent_description(self) -> str:
        return get_agent_description(
            os.getenv("PRISON_MIKE_DESCRIPTION_VERSION", "v1")
        )
    model_id: str = field(
        default_factory=lambda: os.getenv("PRISON_MIKE_MODEL_ID", "gemini-2.0-flash-lite")
    )
